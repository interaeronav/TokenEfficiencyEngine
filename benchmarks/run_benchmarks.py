...
def run_extract_scenario(wire=None) -> tuple | None:
    try:
    ...
    project.mkdir()
    # A68 P1e made `bl_build_from_plan` find its lane BY CAPABILITY
    # (`hasattr(adapter, "execute_python")`), and FakeAdapter has none - so
    # with a fake named "blender" the Phase 8 conformance loop below died with
    # "needs a Blender lane and none is served (served: blender)", aborting the
    # whole battery. It went unseen because this row only runs when the extract
    # extra is installed, and the machine that last re-measured it did not have
    # one. Measured 2026-09-10 on the owner's Mac, where it does.
    blender = BlenderAdapter(wire) if wire is not None else FakeAdapter()
    app = TeeApp({"blender": blender}, project_root=project, allow_code_exec=wire is not None)
...
def main() -> None:
    ...
    try:
        for name, scenario in SCENARIOS:
            ...
            _stage(name, t_s)
        # inside the try: this row needs a lane that can execute Python (see
        # run_extract_scenario), and `finally` below kills the bridge.
        extract_row = _timed(run_extract_scenario, wire)
    finally:
        proc.terminate()
...
#: A per-run stamp written by write_results next to a carried section, e.g.
#: "_Flight dynamics: re-measured this run at 835 tokens over 5 calls._"
_RUN_STAMP = re.compile(r"^_[A-Z][^:]*: (re-measured this run|scenario HELD)")


def _carry_forward(section_header: str, stamped: bool = True) -> list[str]:
    """Preserve a previously-recorded RESULTS.md section whose scenario
    skipped this run (SI-B5: a re-run on a machine without a DCC must not
    erase measured evidence). The kept block is stamped as not re-run."""
    out = Path(__file__).parent / "RESULTS.md"
    if not out.exists():
        return []
    text = out.read_text().splitlines()
    start = next((i for i, line in enumerate(text) if line.startswith(section_header)), None)
    if start is None:
        return []
    end = next(
    ...
    kept = [
        ...
        # Drop the stamp (re-added below) AND the generated-by footer: the
        # footer lives at the end of the file, so whichever section is last
        # swallows it into its carried block and the next run appends a
        # fresh one. Unstripped, every no-DCC run grew the file by one
        # footer line - three had already accumulated by 0.8.0.
        if not line.startswith("*(not re-run this pass")
        and not line.startswith("*Generated by `benchmarks/run_benchmarks.py`")
        # ...and the caller's own per-run stamp, for the same reason: the A75
        # and A76 rows are stamped AFTER their carried body, so the next run
        # carries that stamp back in and appends another. Measured 2026-09-10:
        # +1 line per run per section, the third instance of this exact bug in
        # this function.
        and not _RUN_STAMP.match(line)
    ]
    while kept and not kept[-1].strip():
        kept.pop()
    if not kept:
        return []
    # Collapse blank lines between the header and the body BEFORE re-adding
    # the stamp. Stripping the old stamp leaves the blank that followed it,
    # so an unstripped body grew this section by one blank line per run -
    # measured at +1/run, 13 already banked in the 0.8.0 file.
    body = kept[1:]
    while body and not body[0].strip():
        body.pop(0)
    if not stamped:
        # the caller stamps it itself, and more precisely (A75/A76 say whether
        # they re-measured and at what cost)
        return ["", kept[0], "", *body]
    stamp = "*(not re-run this pass - scenario skipped on this machine; last measured values kept)*"
    return ["", kept[0], "", stamp, "", *body]
...
def write_results(rows, extract_row=None, asset_row=None, physical_row=None,
    ...
    if extract_row is not None:
        ...
        lines += _carry_forward("## Extraction: ingest-once")
        lines += _carry_forward("## Script lane: the conformance fix loop")
    if asset_row is not None:
        ...
        lines += _carry_forward("## Assets: find-select-place")
    if physical_row is not None:
        ...
        lines += _carry_forward("## Physics: settle cost + variance floor")
    ...
    #
    # A77 wired both scenarios to re-run and to print the stamp below, but
    # never regenerated their section bodies AND never carried them forward -
    # and this file rewrites RESULTS.md wholesale. So the first complete run
    # after A77 DELETED both sections, 80 lines of measured evidence, and said
    # only "re-measured this run at 835 tokens" into the gap where the table
    # had been. Measured 2026-09-10. The bodies are carried here, unstamped,
    # because the loop stamps them itself and more precisely.
    for label, header, row in (
        ("Flight dynamics", "## Flight dynamics: a polar becomes an aircraft", flightdyn_row),
        ("Engine lane", "## Engine lane: is the router's table still true?", engines_row),
    ):
        lines += _carry_forward(header, stamped=False)
        if row is None:
    ...
    if surface_row is not None:
        ...
        saving = 100.0 * (1 - r["wire_tokens"] / r["flat_server_tokens"])
        lines += [
            "",
            ...
            "",
            # A77 corrected this paragraph BY HAND in RESULTS.md and left the
            # generator saying "all seven modules" - so every run since has
            # reverted the fix, dropped the bold from the tool count and
            # deleted the saving figure outright, which is why
            # test_a77_benchmark_canary could not find either. Measured
            # 2026-09-10. The saving is computed here rather than written down.
            f"Attaching **every lane a served TEE has** adds "
            f"**{r['added_by_modules']} tokens** to the",
            f"always-loaded surface - the **{r['n_virtual_tools']}** tools they contribute live",
            f"behind the meta-tools, a **{saving:.1f}%** saving. Reaching one costs "
            f"{r['reach_one_tool']}",
            "tokens (one search + one describe), so the flat design only pays",
            f"off in a session that uses more than ~{payoff} distinct long-tail tools.",
        ]
    else:
        lines += _carry_forward("## Tool surface: progressive disclosure")
    if jurisdiction_row is not None:
        ...
        lines += _carry_forward("## Jurisdiction: legal force per regime")
    ...
    # - and neither section has a scenario here to regenerate it. Carried
    # unconditionally: they are never "re-run this pass" by THIS runner.
    lines += _carry_forward("## Scheduler: the mixed-load row")
    lines += _carry_forward("## The pipeline lane: two real projects")
    lines += _carry_forward("## The headless fleet: compact answers")
    # A78's row is the same shape: written by the run_a78_* siblings against a
    # live local model, with no scenario here to regenerate it. It was recorded
    # on 2026-09-10 and was NOT in this list, so the next run of this file
    # would have deleted 75 lines of measured evidence without a word.
    lines += _carry_forward("## Local model lane control")
    lines += [
        "",
