...
| [71-fusion-lane.md](71-fusion-lane.md) | The Fusion lane (A69): Autodesk Fusion as a live GUI lane on the owner's document - the add-in bridge that marshals every request onto Fusion's primary thread through custom events (Autodesk's own MCP sample's mechanism), one script per batch on the FreeCAD precedent, twenty reference-verified API rows with a live column for the Mac smoke (setDistanceExtent retired; export constructors differ in argument order by format; cm inside and mm on the wire with the unit always written), short session-stable ids over entityToken, timeline-plus-parameters checkpoints that say what they restore, budgeted viewport capture, the fu_* long tail, the connected-lane router refinement, and what v1 deliberately leaves out. A71 (2026-09-06) verified it live on Fusion 2704.1.53 over the FusionMcpBridge — 49 live cells, §8.2 and §9 read the facts out, and row 54 records the measured crash that made the id map per document |
| [72-wind-tunnel-lane.md](72-wind-tunnel-lane.md) | Wind-tunnel lane (A72, `wt_*`): OpenFOAM, SU2, OpenVSP/VSPAERO and ParaView at arm's length (every engine a separate process; foamlib GPL-3.0-only, PyFoam, fluidfoam, vtk, pyvista, the NOSA OpenVSP Python bundle all banned by a load-bearing test; meshio + numpy the only extra, in one function), the P0 table measured through the registry (apt OpenFOAM 1912's dead function objects and the move to openfoam.com's v2606; `k_openfoam` 2.36 µs per cell-iteration; the 2-D slab's Aref; 1/2/4 cores 7.8/6.8/4.8 s and Open MPI's root refusal; SU2 8.4.0's QuickStart figure on the official mesh and TEE's O-mesh +1.8 % / −7.3 %; VSPAERO's two VLM facts, exit code 2 after success and CL_α −5.5 % from lifting line; pvpython segfaults offscreen without xvfb but samples render-free; a real cancel in 0.05 s; the apt airFoil2D tutorial adopted, run as-is, and reporting forces once the chord is MEASURED from its wall patch), the token cost of every tool, sixteen defects found by building (the near-zero Cm verdict floor, the 16-character key, the double chord division, lRef 1 on a 35 m section → Cl 34), the P0 answers, the Mac / CfdOF / SimFlow rows deferred to an owner session, and the open questions |
| [73-gui-handoff.md](73-gui-handoff.md) | The wind-tunnel GUI handoff (A73): what ParaView and OpenVSP actually accept - `--state` verified at the source, a state file measured at 203,042 bytes naming its case exactly once and round-tripping between processes, ParaView aborting on signal 6 before it parses arguments when there is no display, `vsp` taking the model as a positional argument, writing a state needing no display at all, and the A72 defect that gated the `.foam` stub behind a run; §2.10 adds what running it on the real applications found - a state that reloads at t=0 because `SaveState` carries the animation scene's time, `ColorBy(d, None)` raising on a case with no field arrays, and the SU2 handoff never having run at all (`NameError: ts`) - and §4b keeps the replacement specification the instability scare produced |
| [74-cfmesh-mesher.md](74-cfmesh-mesher.md) | cfMesh in the wind-tunnel lane (A74): the mesher HELYX sells and OpenFOAM.com already ships - `cartesianMesh` measured against snappyHexMesh on one geometry (37,960 cells in 1.6 s against 46,160 in 12.0 s, and snappy's own log admitting 1.32 of 2 boundary layers at 41.6 % of the requested thickness), the skewness failure that is the campaign's blocker, the closed-surface difference that makes an external-aero case need one multi-solid STL, the dictionary keys read off a real v2606 file, and why HELYX itself cannot be integrated. **Closed 2026-09-07:** §2.7 solves on both meshes (cfMesh converged where snappy stalled, Cl 0.01194 against 0.07458 where the true answer is zero) and §2.8 clears the skewness blocker with `surfaceFeatureEdges` → FMS (5.5497206 → 2.0995350, `checkMesh` clean, 0.4 s) which takes the spurious lift to **0.00004** as a side effect; `mesher="auto"` is the default and `TOLERATED_CHECKS` was never widened. **§2.9 (P5)** measures cfMesh's 2-D mesher against the lane's own O-mesh and DECLINES it - Cd roughly doubles and refining the cartesian mesh widens the gap - keeping two facts: the surface rule inverts (the 2-D mesher refuses a closed surface and wants a ribbon) and cfMesh assigns no patch types in either dimension |
| [75-flight-dynamics.md](75-flight-dynamics.md) | Does TEE have a flight model, and what is JSBSim really? Grounding, not a design of record: the library is LGPL-2.0-**or-later** while the wheel's own `jsbsim` console script (`python/JSBSim.py`) is **GPL-3.0-or-later**, so a gate reading PyPI classifiers (`LGPLv2+`) passes a package with a GPL-3 file in it and only a gate reading file headers catches it; the C++ standalone is LGPL again, so the three routes carry three different obligations; 8.8 MB, 60 bundled aircraft never to be vendored, typed (`py.typed` + `.pyi`), no linux-aarch64 wheel; `FGFDMExec` is the whole loop and `FGLinearization` returns A(13x13)/B(13x4) with named states; measured on the owner's Mac at 1,376x real time (7,200 steps of 60 s in 0.044 s), trim held at 5001 ft / 120.0 KCAS / Nz 1.00, the three classic modes falling out by participation factor with the phugoid at 29.56 s against Lanchester's 28.0 (+5.7 %); the raw surface is 2,152x the useful answer; and three API traps found by the probe failing — `reset_to_initial_conditions()` requires its mode, `get_property_catalog()` returns a list, and `print_property_catalog()` writes at the C++ level where `redirect_stdout` captures nothing |
| [76-flight-dynamics-lane.md](76-flight-dynamics-lane.md) | The flight-dynamics lane (A75, `fd_*`): the seam is literal — a `wt_sweep` polar and a JSBSim `<aerodynamics>` table over `aero/alpha-rad` are the same object in two notations — and the lane is the translation, eight virtual tools, zero always-loaded, tabled individually with no family row; the finding that shapes it is that `FGLinearization` on an aircraft with an empty `<propulsion>` **kills the process** (SIGSEGV, rc=-11, no exception) while the same call on a bundled aircraft returns, so `fd_modes` validates first and the lane runs out-of-process regardless; a generated aircraft loads and reads back exactly (Sw 174.3753 ft2, iyy 1342.24 slug-ft2) with **SI accepted on the wire**, `inertia/*` is 0.0 until `run_ic()` on bundled aircraft too, and the state vector is propulsion-dependent (13 with a piston's `Rpm0`, 12 with an electric motor); §2.5 now carries the premise PROVEN in P1 - a generated aircraft trims in seven Newton iterations, both longitudinal modes name themselves by participation and the trim holds a minute - together with why it looked impossible: JSBSim's `do_trim` reports `qdot` when the un-trimmable axis is `udot`, because a turbine sits near 100% N2 until it spools, `run_ic` does NOT reset that spool (which is what makes the lane's own Newton trim possible), and a turbine with no IdleThrust/MilThrust tables segfaults at run_ic rather than reporting |
| [77-engine-lane.md](77-engine-lane.md) | The engine lane (A76, `eng_*`): TEE is a thorough client of local models and never a server of them, and holds no truth about them — `ENGINES` is a table of claims wearing the word `measured` in a string field, with the declared and measured footprints differing in the same line while `may_swap()` uses the declared one; measured drift on 2026-09-07 — the shim serves eight routes and none is `deepseek` while `dsflash` is still the cheapest declared rung, `q35b` IS served and unreachable for want of a `BUILTIN_PROFILES` row, the default profile is dead by name (`model = "tee-coder"`, which `:8080` does not serve), and nothing is answering at all; a defect it surfaced at `router.py:130-135` where `llm_unreachable` and a real verifier kill both record `verified=False`, so doc 55's designated quality alarm is measuring the network; the lane's product is a MEASURED engine row (discover, read the weights' own config, audition against TEE's fixtures under their own verifiers, persist, and let the router order on the file) with reconciliation as a consequence; six tools, stdlib-only, ~200-token digest against a ~21,000-token naive read; and §9 says plainly that reconciliation alone would be a config fix in a lane costume |
| [78-benchmark-truth.md](78-benchmark-truth.md) | The benchmark tells the truth (A77): TEE's stated core metric - tokens per completed task - is the one thing in the repo with no owner, and it has already drifted. A real server serves 210 tools (17 always-loaded + 193 virtual) while `run_benchmarks.py` measures **141**: 52 tools, 27% of the long tail, are invisible to it because `windtunnel`, `flightdyn` and `engines` are attached by `cmd_serve` and not by the harness, so its headline "89.6% saved" is computed over the corpus it can see rather than the one that ships. RESULTS.md carries 26 sections and 85 tabled numbers and no test references the file at all - licences have a gate, the tool surface has nine assertions, trust has a table, and the metric CLAUDE.md judges every decision by has nothing. The design: one source of truth for what a server is, every runnable row re-measured and every held one dated, and a canary on the invariants a change would break silently |
