...
| SimFlow | proprietary freeware; site blocks non-browser fetches (read in a browser by the owner 2026-09-06, `https://sim-flow.com`) | none | **not integrable as software**; free tier is the full engine capped at **200,000 mesh nodes and 1 CPU** for both meshing and computing, with no signup, no card and no time limit — a single-core 200 k-node ceiling is below this lane's working sizes, and there is no API to drive. If it writes a standard OpenFOAM case, that case is adoptable like any other (row S1, closed) |
...
| M1–M8 | **MEASURED 2026-09-06** (owner session on the Mac; PROGRESS "A72 P0b", log `72-evidence/p0b-mac-2026-09-06.log`): entry-script route, native arm64 OpenFOAM/OpenVSP/ParaView, **SU2 x86_64 under Rosetta**, k_openfoam 2.157e-06, pvpython offscreen with no xvfb, CL_alpha 4.9037/rad, the app's own airFoil2D adopted to Cl 0.970 |
| C1–C3 | C1 **measured 2026-09-06**: CfdOF 1.37.3 imports headlessly via `FreeCAD.app/Contents/MacOS/FreeCAD -c` (FreeCAD 1.1 keeps add-ons in a versioned `v1-1/Mod` dir and ships no `FreeCADCmd`). C2/C3 **open**: both need the GUI up. Until then CfdOF is "write with the GUI, adopt with TEE" — the adoption parser is tested on a CfdOF-shaped fixture |
| S1 | **CLOSED 2026-09-06** (owner, in a browser): 200,000 mesh nodes / 1 CPU, no signup, no card, no time limit; not integrable as software, output adoptable — see §2 |
| R1–R4 | R1 lifting line: a textbook formula (Anderson §5.3–5.4), verified as a formula; R2 the SU2 QuickStart figure: measured on the official mesh 2026-09-06 (the AGARD AR-211 cross-check still owed); **R4 CLOSED 2026-09-06**: the NASA TMR moved to `https://tmbwg.github.io/turbmodels/` and its `FlatPlate/SST/cf_plate_sstv.dat` gives Cf 2.690853551e-03 at x = 0.9700840712 m (CFL3D 545×385), now in `verify.REFERENCES`; **R3 CLOSED 2026-09-07** by re-basing rather than by reaching the originals: every route to the two JFM papers failed (Cambridge paywalled and mid-outage, ADS bot-check, DTIC maintenance, and Fornberg's own NASA report NTRS 19840012155 has no drag table), so `cylinder_re40` now targets Gautier, Biau & Lamballais, *Computers & Fluids* **75** (2013) 103–111 — open at `hal.science/hal-00876327`, peer reviewed, purpose-built as a reference solution — at **CD 1.4931, Lw/D 2.2360** with its own convergence study; the historical 1.52 / 1.50 remain context, never the tolerance. **No case is unverified now**. The `cylinder_re40` runner was built 2026-09-07 and passes on live OpenFOAM v2606 (CD 1.518301, +1.69 %; wake Lw/D 2.2791, +1.93 %; Re exactly 40, 18.2 s); `flatplate` was built 2026-09-07 (Cf 2.646754e-03 at x = 0.9700840712, −1.64 % against the TMR's 2.690854e-03, 320 wall-shear samples, 230 s); its geometry and BCs were read from the TMR's own grid and `.nmf` boundary map rather than assumed, and the residual offset is the compressible-vs-incompressible one the TMR itself warns about. **Every reference now has a runner** |
| G1 | §2 |
