...
1. **STILL OPEN after the Mac session (2026-09-07), and the reason is structural.** Does ParaView 6.1 accept a 5.11-written state file? The Mac cannot answer it: it has exactly one ParaView (6.1.1), so it writes and reads with the same install and there is no 5.11 writer here to produce the input. Answering it needs a state carried from a 5.11 machine — the Linux container has the apt 5.11.2 — which is a two-machine test, not a Mac row. It stays open rather than being filled in by hand. It also stays *harmless*: the lane writes with the pvpython that came from the install the GUI launches, so the skew only arises if a state is carried between machines, which nothing in the lane does today.
2. **ANSWERED on the Mac, 2026-09-07: the documented idiom is WRONG as written, and the lane never needed it.** Measured three ways:
   - `open -a ParaView --args --state=<file>` **fails**: *"Unable to find application named 'ParaView'"*. The bundle is `ParaView-6.1.1.app`, and `open -a` matches the application NAME, so a versioned bundle is not found under the bare name. Nothing to do with flag pass-through — it never launched.
   - `open -a /Applications/ParaView-6.1.1.app --args --state=<file>` **works**: the flag passes through (the running process's command line carries `--state=`), and the GUI opens the case at the solution. Note it runs from `AppTranslocation`, a read-only randomised copy macOS uses for a quarantined bundle.
   - **`wt_open`'s own argv — the direct binary with `--state=<file>` — works**, and is what the lane actually composes. It does not use `open` at all, so the wrong idiom in this document never reached the product.

   Confirmed by looking at the window, not by exit codes: the pipeline browser holds `OpenFOAMReader1`, the toolbar reads `Time: 197`, and the colour legend is titled `p` over −6.3e+02…4.5e+02. One usability observation while it was open: the camera frames the whole O-mesh farfield (50 chords), so the domain renders as a near-uniform disc with the aerofoil a speck at the centre. The state is correct; the framing is not useful, and a `wt_open` that fitted the camera to the wall patch would be worth having.
