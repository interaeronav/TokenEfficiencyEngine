...
from drafting import loop, standards as S
from drafting.compose import compose, INK, GREY, ACCENT, MUTED, FAINT
...
WORK=SCRATCH/"okongo-work"; OUT=Path.home()/"Downloads/Okongo-Scan-Test"; OUT.mkdir(parents=True,exist_ok=True)
lid=json.loads((WORK/"outputs.json").read_text())["lid"]
P=CloudStore(WORK).points(lid)
band=slice2d.band(P,1.20,0.05); RAW,_=slice2d.fit_ortho(band)
...
for b in BODIES:
    for s_ in SEGS:
        ...
        if pos is not None and min(abs(pos-b.a[0]),abs(pos-b.d[0]))<0.02: POCHED.add(id(s_))
        pos=(s_["a"][1]+s_["b"][1])/2 if abs(s_["a"][1]-s_["b"][1])<0.05 else None
        if pos is not None and min(abs(pos-b.a[1]),abs(pos-b.d[1]))<0.02: POCHED.add(id(s_))
print(f"linework: {len(SEGS)} faces, {len(BODIES)} poche bodies")

W_W,W_E,W_S,W_N = -3.157, 1.568, -2.342, 1.610
R2_E, R2_N, R1_W = -1.660, 1.080, -1.315
CUT_A=((-3.45,-1.80),(1.85,-1.80)); CUT_B=((0.95,-2.60),(0.95,2.60))

# --- the opening chain the corrector refused to invent, measured instead ----
def openings(segs, x_target, tol=0.09):
    runs=sorted((min(s["a"][1],s["b"][1]), max(s["a"][1],s["b"][1]))
                for s in segs if abs((s["a"][0]+s["b"][0])/2 - x_target)<tol
                and abs(s["a"][0]-s["b"][0])<0.05)
    gaps=[]
    for (_,e),(s2,_) in zip(runs, runs[1:]):
        if s2-e > 0.30: gaps.append((e,s2))
    return gaps
DOOR = max(openings(SEGS,-1.54)+openings(SEGS,-1.66), key=lambda g:g[1]-g[0], default=None)
print("door opening measured:", None if DOOR is None else f"{(DOOR[1]-DOOR[0])*1000:.0f} mm at y {DOOR[0]:.2f}..{DOOR[1]:.2f}")

# --- run the loop ----------------------------------------------------------
dset=as_issued()
before=critique(dset)
res=loop.run(dset,
  project={"project":"Okongo Oneleiwa","client":"J. Nangolo (owner)","scale":"",
           "date":"2026-09-04","drawn_by":"TEE pc_* lane (A67)","checked_by":""},
  rooms={"ROOM 01":"BEDROOM","ROOM 02":"EN-SUITE / STORE"},
  cuts={"A":("SK-01",270.0),"B":("SK-01",180.0)},
  provenance=("Measured by iPhone LiDAR (3D Scanner App 2.5), 1,520,736 points; levelled to "
              "0.0000 deg residual, floor-plane RMS 12.3 mm. SCALE UNVERIFIED - no tape baseline."))
# the measured chain closes the last finding honestly
if DOOR: dset.by_number("SK-01").views[0].dimension_chains.append("opening")
after=critique(dset)
print(f"\nloop: {len(before)} -> {len(after.open)} open findings, {len(after.blocking)} blocking")
for f in after.open: print("   surviving:", f.line())

SK={s.number:s for s in dset.sheets}
for s in dset.sheets:
    s.title_block.fields["scale"] = ("1:%d @ A3" % s.views[0].scale_denominator
                                     if s.views[0].kind!="pictorial" else "NTS @ A3")

def dim(ax,p1,p2,txt,off,vertical=False,size=7.1):
    p1=np.array(p1,float); p2=np.array(p2,float)
    n=np.array([off,0.]) if vertical else np.array([0.,off])
    a,b=p1+n,p2+n
    ax.annotate("",xy=b,xytext=a,zorder=8,arrowprops=dict(arrowstyle="<|-|>",color=MUTED,
                lw=cv.pen("hatch"),mutation_scale=6,shrinkA=0,shrinkB=0))
    for q,p in ((a,p1),(b,p2)):
        ax.plot([p[0],q[0]],[p[1],q[1]],color=MUTED,lw=cv.pen("hatch"),ls=(0,(2,2)),zorder=7)
    m=(a+b)/2
    ax.text(m[0],m[1],txt,ha="center",va="center",fontsize=size,color=INK,zorder=9,
            rotation=(90 if vertical else 0),bbox=dict(fc="white",ec="none",pad=1.2))

# =============================================================== SK-01 PLAN
sheet=SK["SK-01"]; V=sheet.views[0]; SC=V.scale_denominator
lo=np.array([P[:,0].min()-0.55,P[:,1].min()-0.75]); hi=np.array([P[:,0].max()+0.55,P[:,1].max()+0.55])

def plan_body(canvas):
    global cv; cv=canvas
    plan_w=(hi[0]-lo[0])*1000/SC; plan_h=(hi[1]-lo[1])*1000/SC
    ax,aw,ah=canvas.view_axes((8+274)/2-plan_w/2, (86+264)/2-plan_h/2,
                              ((lo[0],lo[1]),(hi[0],hi[1])), SC)
    P_ = lambda cat, cut=False: S.resolve_pen(cat, SC, cut=cut)*S.POINTS_PER_MM
    for g in np.arange(np.ceil(lo[0]),hi[0],1.0): ax.axvline(g,color=FAINT,lw=P_("grid"),zorder=0)
    for g in np.arange(np.ceil(lo[1]),hi[1],1.0): ax.axhline(g,color=FAINT,lw=P_("grid"),zorder=0)
    # the survey evidence is HALFTONE - present, legible, plainly background
    ax.scatter(band[:,0],band[:,1],s=.4,c=[S.halftone(GREY,0.55)],linewidths=0,zorder=1)
    ...
    for s in SEGS:
        ...
        ax.plot([s["a"][0],s["b"][0]],[s["a"][1],s["b"][1]],color=INK,
    ...
    if DOOR:
        dim(ax,(R2_E,DOOR[0]),(R2_E,DOOR[1]),f"{(DOOR[1]-DOOR[0])*1000:.0f}",-0.62,vertical=True,size=6.8)
        ax.annotate("OPENING",xy=(R2_E-0.05,(DOOR[0]+DOOR[1])/2),xytext=(-2.72,1.55),
                    fontsize=7.1,color=ACCENT,zorder=10,ha="center",
    ...
    for r,cx,cy in ((V.rooms[0],(R1_W+W_E)/2,-0.55),(V.rooms[1],(W_W+R2_E)/2,-0.55)):
        ax.text(cx,cy,r.name,ha="center",fontsize=9.9,weight="bold",color=INK,zorder=9,bbox=mask)
        ax.text(cx,cy-0.30,r.number,ha="center",fontsize=7.1,color=MUTED,zorder=9,bbox=mask)
        ax.text(cx,cy-0.56,f"{r.area_m2:.1f} m²",ha="center",fontsize=7.1,color=MUTED,zorder=9,bbox=mask)
    ax.text(-0.55,2.52,"LOBBY / OPENING",ha="center",fontsize=7.1,weight="bold",color=INK,zorder=9,
            bbox=dict(fc="white",ec="none",pad=1.0))
    canvas.north_point(ax,hi[0]-0.45,hi[1]-0.75,0.40)
    canvas.scale_bar(158,20,SC)
    canvas.notes_panel(30,76,[
      ...
      "", sheet.provenance[:96], sheet.provenance[96:]])
cv1=compose(sheet, plan_body)
from drafting.legibility import inspect
leg=inspect(cv1)
print(f"\nSK-01 legibility: {len(leg)} findings")
for f in leg.findings: print("   ", f.line())
cv1.save(OUT/"01-floor-plan.pdf", OUT/"01-floor-plan.png")
print("SK-01 re-issued at 1:%d" % SC)

# ============================================================ SK-02 SECTIONS
sheet2=SK["SK-02"]; SC2=sheet2.views[0].scale_denominator
def sec_body(canvas):
    global cv; cv=canvas
    for k,(view,cut,caption) in enumerate(zip(sheet2.views,(CUT_A,CUT_B),
        ...
        ax,aw,ah=canvas.view_axes(32, canvas.h-58-k*104-(3.85*1000/SC2),
                                  ((-0.35,-0.40),(L+0.35,3.45)), SC2)
        ax.scatter(sec[:,0],sec[:,1],s=.55,c=GREY,alpha=.75,linewidths=0,zorder=1)
        for lvl,lab in ((0.0,"FFL  ±0.000"),(2.604,"SOFFIT  +2.604")):
            ax.plot([-0.25,L+0.25],[lvl,lvl],color="#c0392b",lw=canvas.pen("beyond"),zorder=4)
        ...
        ax.text(0,3.10,f"{caption}   ·   cut line shown on SK-01",fontsize=7.1,color=MUTED,va="top")
    canvas.scale_bar(150,30,SC2)
    canvas.notes_panel(32,44,[sheet2.level_datum, sheet2.provenance[:92], sheet2.provenance[92:]])
cv2=compose(sheet2, sec_body)
leg2=inspect(cv2); print(f"SK-02 legibility: {len(leg2)} findings")
for f in leg2.findings: print("   ", f.line())
...
VIEWS=[("v_cutaway.png","CUTAWAY AXONOMETRIC","ceiling removed above +2.35 m"),
...
def d3_body(canvas):
    ...
    for (fn,cap,sub),(x,y,w_,h_) in zip(VIEWS,grid):
        ...
        im=Image.open(src); im.thumbnail((int(w_*8),int(h_*8)))
        a=canvas.fig.add_axes([x/canvas.w,y/canvas.h,w_/canvas.w,h_/canvas.h])
        a.imshow(np.asarray(im)); a.axis("off")
        canvas.text(x,y+h_+6.0,cap,"subtitle",weight="bold")
        canvas.text(x,y+h_+1.6,sub,"note",color=MUTED)
    canvas.text(24,17,"NOT A MESH AND NOT A RECONSTRUCTION — these are the measured points, "
                "drawn with a depth buffer.","note",weight="bold")
    canvas.text(24,12,"Where the scan did not reach the drawing is empty, and that absence "
                "is information.  "+sheet5.provenance[:60],"note",color=MUTED)
cv5=compose(sheet5, d3_body)
leg5=inspect(cv5); print(f"SK-05 legibility: {len(leg5)} findings")
for f in leg5.findings: print("   ", f.line())
cv5.save(OUT/"05-point-cloud-views.pdf", OUT/"05-point-cloud-views.png", dpi=175)

# ------------------------------------------------------------- the record
final=critique(dset)
lines=["# Drafting critique — Okongo as-scanned set","",
...
for f in res.changes:
    ...
    seen.add(key); lines.append(f"- `{f.rule}` {f.where} — {f.detail}")
lines += ["","## What a human still has to do","",
...
from drafting.spec import Sheet, View, Text, Line, TitleBlock
from matplotlib.patches import Polygon as MplPoly

R1 = dict(w=-1.315, e=1.568, s=-2.225, n=1.610)
ELEVS = [("E1", "NORTH wall of Room 01", 1, R1["n"], R1["w"], R1["e"], -1.0),
...
def _new_sheet(number, title, subtitle, views):
    src = SK["SK-01"]
    return Sheet(number=number, title=title, subtitle=subtitle, views=views,
                 texts=[Text(t.role, t.height_mm) for t in src.texts],
                 lines=[Line(l.role, l.width_mm) for l in src.lines],
                 title_block=TitleBlock(fields=dict(src.title_block.fields),
                                        notes=list(src.title_block.notes)),
                 level_datum=SK["SK-02"].level_datum, provenance=src.provenance)

sheet3 = _new_sheet("SK-03", "INTERNAL ELEVATIONS — as-scanned survey",
...
def elev_body(canvas):
    ...
    for k,(tag,name,axis,pos,lo,hi,look) in enumerate(ELEVS):
        ...
        ax.scatter(pts[:,0], pts[:,1], s=.35, c=[S.halftone(GREY,0.5)], linewidths=0, zorder=1)
        for lvl in (0.0, 2.604):
            ax.plot([-0.2,width+0.2],[lvl,lvl], color="#c0392b", lw=P_("floor",cut=True),
                    zorder=4)
        ...
        ax.text(width, -0.16, f"{width*1000:.0f}", fontsize=7.1, color=MUTED,
                ha="right", va="top")
    canvas.scale_bar(300, 96, SC3)
    canvas.notes_panel(26, 78, [
        ...
        "", sheet3.level_datum, "", sheet3.provenance[:96], sheet3.provenance[96:]])
cv_e = compose(sheet3, elev_body)
leg_e = inspect(cv_e); print(f"SK-03 legibility: {len(leg_e)} findings")
for f in leg_e.findings: print("   ", f.line())
cv_e.save(OUT/"03-internal-elevations.pdf", OUT/"03-internal-elevations.png")

# ================================================= SK-04 DRAWN AXONOMETRIC
SC4 = 50
QUADS = V.wall_quads(SEGS, P)
sheet4 = _new_sheet("SK-04", "AXONOMETRIC — as-scanned survey",
...
def axo_body(canvas):
    ...
    for k,(az,el,label) in enumerate([(45,28,"from the SOUTH-EAST"),(135,28,"from the SOUTH-WEST")]):
        M = V.iso_matrix(az, el)
        ordered = V.painter_order(QUADS, M)
        proj = [V.project(q["corners"], M) for q in ordered]
        allp = np.vstack(proj)
        pad = 0.30
        ax,_,_ = canvas.view_axes(24 + k*176, 108,
            ((allp[:,0].min()-pad, allp[:,1].min()-pad),
             (allp[:,0].max()+pad, allp[:,1].max()+pad)), SC4)
        # floor first, then the walls back to front - solids hide what is behind
        floor = np.array([[P[:,0].min(),P[:,1].min(),0],[P[:,0].max(),P[:,1].min(),0],
                          [P[:,0].max(),P[:,1].max(),0],[P[:,0].min(),P[:,1].max(),0]])
        ax.add_patch(MplPoly(V.project(floor, M), closed=True, fc="#f2f3f4",
                             ec=S.halftone(INK,0.55), lw=P_("floor"), zorder=1))
        for q, xy in zip(ordered, proj, strict=True):
            heavy = q["length_m"] >= 0.9
            ax.add_patch(MplPoly(xy, closed=True, fc="white",
                                 ec=INK, lw=P_("wall", cut=heavy), zorder=2))
        ax.text(allp[:,0].min(), allp[:,1].max()+pad*0.6, label,
                fontsize=7.1, color=MUTED, va="bottom")
    canvas.notes_panel(24, 92, [
      "HOW THIS IS BUILT",
      ...
      "  than being stretched to the ceiling. %d faces became %d solids." % (len(SEGS), len(QUADS)),
      ...
      "", sheet4.provenance[:96], sheet4.provenance[96:]])
cv4 = compose(sheet4, axo_body)
leg4 = inspect(cv4); print(f"SK-04 legibility: {len(leg4)} findings")
for f in leg4.findings: print("   ", f.line())
