...
5. **A state round-trips.** Written by one process and loaded by a fresh `pvpython`, the reader comes back with its `FileName`, the view with `ViewTime` 0.0 and `CameraPosition` `[0.5, 0.0, 273.224]`, and the representation still coloured by `['POINTS', 'p']`. **That `ViewTime` 0.0 is a defect, not a fact about state files** — it was recorded here as a round trip and P2 found it on a solved case; see the amendments below.
...
## Amendments learned while building (2026-09-07; the script is amended, not improvised around)

- **P2 defect 1 — the state opened at t=0.** `SaveState` carries the **animation scene's** time, not the view's, so `rv.ViewTime = ts[-1]` alone was lost on reload: ParaView opened the initial field, coloured and framed exactly like the solution. `state_script` now sets `GetAnimationScene()` → `UpdateAnimationUsingDataTimeSteps()` → `AnimationTime`, in both kinds, and a live test asserts the reloaded time equals the case's last time step. Fact 5 above recorded the symptom on 2026-09-07 and read it as a success.
- **P2 defect 2 — `wt_open view=mesh` failed on a case that had not run.** `ColorBy(d, None)` re-reads the representation's current association, which is `'NONE'` when the data carries no array to colour by, and raises `invalid association string 'NONE'`. `ColorBy(d, ('CELLS', None))` names a valid association and works either way. `paraview.render_script` keeps the old form deliberately: `wt_view` refuses without a run, so its data always has arrays — the defect belongs to the one path that works without a run.
- **P2 sizes, after both fixes** (ParaView 5.11.2 + xvfb, 16,000-cell case at 197 iterations): full state over a solved case **206,584 bytes**, written 4.5 s, read back 3.6 s at t=197 with `['CELLS', 'p']`; full state over a meshed case with no run **179,725 bytes**; pipeline state **17,233 bytes**, written and read with no display in either direction. `relocate()` replaced exactly **1** occurrence over a copied run and the moved state loaded 16,000 cells from the new path.
- **P2 defect 3 — `wt_open` on an SU2 case raised `NameError: ts`.** `paraview._reader_lines` bound `ts` in its `.foam` branch and not in its `.vtu` one, while `state_script` reads it in both: the handoff worked for one engine and had never once run for the other. The `.vtu` branch now binds `ts` to an empty list (a single `.vtu` has no time steps), and a hermetic test asserts the reader block assigns `ts` before anything reads it, for both sources and both kinds. Full state over a `.vtu`: 198,796 B; pipeline: 12,089 B.
- **P2 OpenVSP:** the route is verified by asking OpenVSP — `vsp -script` reading back the `.vsp3` the command line names reports `GEOMS=1`, `WingGeom`, type `Wing` — rather than by the file existing. The AngelScript name is `FindGeoms()`; `GetGeomIDs` does not exist.
- **P2 test-helper trap, twice now:** a helper `call(app, tool, **args)` collides with `wt_open`'s own `app` argument. Both `test_windtunnel_live.py` and the panel's tests take the argument positionally (`def call(app, tool, /, **args)`).
- **ParaView's instability, raised mid-campaign, was reported resolved by the owner's local session on 2026-09-07.** No swap is being made; doc 73 §4b keeps the three instabilities measured here and the five-row specification a replacement would have to satisfy. The narrowing this campaign accepted while the question was open would have cost it both P2 defects.
