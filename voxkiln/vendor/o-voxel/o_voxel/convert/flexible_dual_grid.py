...
# voxkiln vendor surgery: the compiled extension is optional. Mesh
# extraction (flexible_dual_grid_to_mesh) has an exact portable fallback
# below; mesh_to_flexible_dual_grid (training data prep) still needs _C.
try:
...
def _pack4(c: torch.Tensor) -> torch.Tensor:
    c = c.long() + (_KEY_SPAN // 2)
    return ((c[:, 0] * _KEY_SPAN + c[:, 1]) * _KEY_SPAN + c[:, 2]) * _KEY_SPAN + c[:, 3]
...
def flexible_dual_grid_to_mesh(
    ...
    # Find connected voxels. The CUDA hashmap is used when available; the
    # voxkiln fallback is an exact sorted-key searchsorted lookup that runs
    # on any device (MPS, CPU, CUDA) with identical results.
    ...
    if _HAS_C and coords.is_cuda:
        ...
        keys = _pack4(torch.cat([torch.zeros_like(coords[:, :1]), coords], dim=-1))
        order = keys.argsort()
        sorted_keys = keys[order]
        probe = connected_voxel.reshape(-1, 3)
        probe_keys = _pack4(torch.cat([torch.zeros((M * 4, 1), dtype=probe.dtype, device=probe.device), probe], dim=1))
        pos = torch.searchsorted(sorted_keys, probe_keys).clamp(max=max(N - 1, 0))
        found = (sorted_keys[pos] == probe_keys).reshape(M, 4)
        connected_voxel_indices = order[pos].reshape(M, 4)
        connected_voxel_valid = found.all(dim=1)
        quad_indices = connected_voxel_indices[connected_voxel_valid].int()                         # (L, 4)
