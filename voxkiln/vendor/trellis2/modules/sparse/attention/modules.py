...
def sparse_windowed_scaled_dot_product_self_attention(*args, **kwargs):
    # voxkiln vendor surgery: windowed_attn.py dropped - no shipped
    # TRELLIS.2 config uses attn_mode other than 'full'.
    raise NotImplementedError(
        "windowed sparse attention is not in the voxkiln vendor tree "
        "(no shipped config uses it; attn_mode is always 'full')"
    )
