...
def sparse_scaled_dot_product_attention(*args, **kwargs):
    ...
    if config.ATTN == 'xformers':
        ...
        # voxkiln: portable exact backend for MPS/CPU. Each variable-length
        # sequence gets its own dense SDPA call - no padding, so no
        # zero-key attention mass (the correctness wrinkle in trellis-mac's
        # padded shim). The batch is almost always 1 here, so the loop is
        # a single fused SDPA dispatch in practice.
        ...
        for lq, lkv in zip(q_seqlen, kv_seqlen):
            qi = q[q_off:q_off + lq].permute(1, 0, 2).unsqueeze(0)      # [1, H, Lq, Ci]
            ki = k[kv_off:kv_off + lkv].permute(1, 0, 2).unsqueeze(0)   # [1, H, Lkv, Ci]
            vi = v[kv_off:kv_off + lkv].permute(1, 0, 2).unsqueeze(0)   # [1, H, Lkv, Co]
            oi = F.scaled_dot_product_attention(qi, ki, vi)
            outs.append(oi.squeeze(0).permute(1, 0, 2))                 # [Lq, H, Co]
            q_off += lq
            kv_off += lkv
