...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    def sample_sparse_structure(
        ...
        noise = torch.randn(num_samples, in_channels, reso, reso, reso,
                            generator=getattr(self, '_noise_generator', None)).to(self.device)
    ...
    def sample_shape_slat(
        ...
        noise = SparseTensor(
            feats=torch.randn(coords.shape[0], flow_model.in_channels, generator=getattr(self, '_noise_generator', None)).to(self.device),
            coords=coords,
        )
    ...
    def sample_shape_slat_cascade(
        ...
        # LR
        noise = SparseTensor(
            feats=torch.randn(coords.shape[0], flow_model_lr.in_channels, generator=getattr(self, '_noise_generator', None)).to(self.device),
            coords=coords,
        )
        sampler_params = {**self.shape_slat_sampler_params, **sampler_params}
        if self.low_vram:
        ...
        while True:
            ...
            # voxkiln: '<= 1024' fixes the upstream loop guard (== missed
            # non-multiple-of-128 paths; stableprojectorz correction), and
            # the downgrade is recorded so callers can report it loudly
            # instead of it hiding in stdout (research 44 fix list #12).
            if num_tokens < max_num_tokens or hr_resolution <= 1024:
                if hr_resolution != resolution:
                    print(f"Due to the limited number of tokens, the resolution is reduced to {hr_resolution}.")
                    if hasattr(self, 'last_run_report'):
                        self.last_run_report['resolution_downgraded'] = {
                            'requested': resolution, 'actual': hr_resolution,
                            'reason': f'token budget {max_num_tokens}',
                        }
                break
            hr_resolution -= 128
    ...
    def sample_tex_slat(
        ...
        noise = shape_slat.replace(feats=torch.randn(shape_slat.coords.shape[0], in_channels - shape_slat.feats.shape[1], generator=getattr(self, '_noise_generator', None)).to(self.device))
    ...
    def run(
        ...
        # voxkiln (research 48): noise comes from a dedicated CPU generator,
        # so the same seed yields the same noise on CUDA, MPS, and CPU -
        # upstream's bare torch.manual_seed left it to per-device RNG.
        torch.manual_seed(seed)
        self._noise_generator = torch.Generator().manual_seed(seed)
        self.last_run_report = {"seed": seed, "pipeline_type": pipeline_type}
        ...
        if hasattr(self, 'last_run_report'):
            self.last_run_report['resolution'] = res
